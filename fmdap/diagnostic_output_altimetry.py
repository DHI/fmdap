...
class DiagnosticOutputAltimetry:
    ...
    def read(self, file_diag, file_obs, obs_col_name='adt_dhi'):
        ...
        self.dfd = Dfs0(file_diag).to_dataframe()
        self.dfd['ModelTime'] = self.dfd.index
        cols = self.dfd.columns.values
        for j, col in enumerate(cols):
            if col.startswith('Measurement'): col = 'Measurement'
            if col.startswith('Mean State'): col = 'Mean_State'
            cols[j] = col
        self.dfd.columns = cols
        
        # Load original observation track data
        dfo = Dfs0(file_obs).to_dataframe() #self._dfs.
    ...
    def process(self, col_no_DA='mike_wl', track_split_time=3):
        ...
        is_analysis = [self.has_duplicate_txy_before(tvec, xvec, yvec, j) for j in range(nrows)]
        is_forecast_at_DA = [self.has_duplicate_txy_after(tvec, xvec, yvec, j) for j in range(nrows)]
    ...
    def read_dfda(self, filename):
        ...
        tmp = (dfda.loc[dfda.super_obs.notnull(),'super_obs'] - dfda.loc[dfda.super_obs.notnull(),'obs']).values.astype('float')
    ...
    def create_dfda(self, col_no_DA='mike_wl'):
        ...
        cols = ['Longitude', 'Latitude', 'model_time', 'mean_f', 'std_f', 'mean_a', 'std_a', 'no_DA', 'obs', 'super_obs', 'track_id']
        ...
        dfda.loc[n_DA_pts>0,['model_time','mean_f','std_f','super_obs']] = dff.values
        dfda.loc[n_DA_pts>0,['mean_a','std_a']] = dfa.values

        # estimate offset and apply to obs
        tmp = (dfda.loc[n_DA_pts>0,'super_obs'] - dfda.loc[n_DA_pts>0,'obs']).values.astype('float')
        offset = np.round(np.nanmedian(tmp),7)
        print(f'Estimated offset: Median difference between obs and superobs, {offset}')
        dfda['obs'] = dfda['obs'] + offset 
    ...
    def create_dfqa(self, col_no_DA='mike_wl'):
        ...
        dfd = self.dfd
        # for each row of dfd find corresponding row (index) in dfo
        cols = ['Longitude', 'Latitude', 'model_time', 'mean_f', 'std_f', 'no_DA', 'obs', 'super_obs', 'track_id']
        ...
        nearest_idx = [get_mid_elem(np.where(idx2==j)[0]) for j in np.unique(idx2[idx2>=0])]
        ...
        # set new values vectorized: 
        dff = dff.drop(columns=['obs_pt_idx'])
        dfqa.loc[n_QA_pts>0,['model_time','mean_f','std_f','super_obs']] = dff.values
        
        # estimate offset and apply to obs
        tmp_diff = (dfqa.loc[n_QA_pts>0,'super_obs'] - dfqa.loc[n_QA_pts>0,'obs']).values.astype('float')
    ...
    def plot_track(self, track_id):
        ...
        ax1 = plt.gca()
        ax1.fill_between(dfsub.index, (dfsub.mean_f-dfsub.std_f).astype(float), (dfsub.mean_f+dfsub.std_f).astype(float), facecolor='blue', alpha=.2)
        ax1.fill_between(dfsub.index, (dfsub.mean_a-dfsub.std_a).astype(float), (dfsub.mean_a+dfsub.std_a).astype(float), facecolor='orange', alpha=.2)
    ...
    def _plot_track_in_axis(self, dfsub, ax):
        ...
        ax.fill_between(dfsub2.index, (dfsub2.mean_f-dfsub2.std_f).astype(float), (dfsub2.mean_f+dfsub2.std_f).astype(float), facecolor='blue', alpha=.2)
        ax.fill_between(dfsub2.index, (dfsub2.mean_a-dfsub2.std_a).astype(float), (dfsub2.mean_a+dfsub2.std_a).astype(float), facecolor='orange', alpha=.2)
    ...
    def _plot_map_in_axis(self, dfsub, ax):
        ...
        ax.plot(dfsub.Longitude[0], dfsub.Latitude[0], color='black', marker='o', markersize=8, linestyle='None')

    def get_unique_model_times(self, dfsub):
        model_times = pd.to_datetime(np.unique(dfsub[dfsub.super_obs.notnull()].model_time.values))
        return model_times
    ...
    def plot_track_with_map(self, track_id, title=None, figsize=None, show_statistics=True, unit='m'):
        dfsub = self.dfda[self.dfda.track_id == track_id]
        dfsub2 = dfsub.dropna(subset=['model_time'])        
        nDA = len(dfsub2)
        if nDA == 0:
        ...
        fig, (ax1, ax2) = plt.subplots(1,2, gridspec_kw={'width_ratios': [2, 1]}, figsize=figsize)
