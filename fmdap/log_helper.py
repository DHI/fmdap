...
def combine_f_a(df_f, df_a, namef='f', namea='a'):
    cols_f = ['name','n_points_'+namef,'bias_'+namef,'rmse_'+namef,'max_err_'+namef,'stdev_'+namef]
    df_f.columns = cols_f
    cols_a = ['name_a','n_points_'+namea,'bias_'+namea,'rmse_'+namea,'max_err_'+namea,'stdev_'+namea]
    df_a.columns = cols_a        
    df = pd.concat([df_f, df_a], 1).dropna() #.mean(axis=1, level=0)
    cols = ['name','n_points_'+namef,'n_points_'+namea,'bias_'+namef,'bias_'+namea,'rmse_'+namef,'rmse_'+namea,'max_err_'+namef,'max_err_'+namea,'stdev_'+namef,'stdev_'+namea]
    return df[cols]
...
def stats_section_to_df(all_lines, search_str):
    ...
    df = pd.read_csv(io.StringIO(lines_as_str), sep='\s+', header=None, names=cols, index_col=0)
    return df
...
def get_lines_in_stats_section(all_lines, search_str):
    ...
    if idx is None:
        return None
    idx1 = idx + 4
    idx2 = idx1 + first_line_idx(all_lines[idx1:], "-------------------------------------")
